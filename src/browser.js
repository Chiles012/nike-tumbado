...
function startPuppeteer(puppeteerParams) {
    ...
    const size_config = (puppeteerParams.windowSize && puppeteerParams.windowSize.width > 0 && puppeteerParams.windowSize.heigth > 0) ? `--window-size=${puppeteerParams.windowSize.width},${puppeteerParams.windowSize.height}` : `--window-size=${200},${200}`;
    ...
    puppeteer.launch({ headless: puppeteerParams.headless, devtools: puppeteerParams.devtools, ignoreHTTPSErrors: true, args: [size_config], executablePath: config.chromePath }).then(async browser => {
        ...
        /*if ( !puppeteerParams.dis_intercept ) {
            console.log(puppeteerParams.dis_intercept);
